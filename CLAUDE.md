...
**Project hash** (bridge scope): 8-char SHA-1 of the project identity (`getProjectHash(transcriptPath)`), stable across conversations. The journal is conversation-scoped (per-conversation why-chains), but the ctx_bridge is **project-scoped** because its job is to hand recovery context from a finished conversation to the *next, different* one — which has a new conversation hash. Keying the bridge by conversation hash (v0.7.0–v0.8.0) made that handoff impossible on a fresh start; v0.8.1 restored project-scoping.

The project identity MUST be identical for the reader (SessionStart) and the writer (SessionEnd/PreCompact/tracker), or they look under different keys and the handoff silently fails. v0.8.1 derived it from the git root of the hook's **cwd**, which is not stable — it drifts between hook invocations, and a **nested git repo** inside the workspace makes `git rev-parse` resolve a different root than the workspace. v0.8.2 fixed this by deriving the key from the **project slug in the transcript path** (`~/.claude/projects/<slug>/<uuid>.jsonl`) — the harness's own per-project id, delivered identically to every hook on stdin, cwd-independent. Resolution order: `MEMENTO_PROJECT_HASH` override → transcript slug (`projectSlugFromTranscript`) → `CLAUDE_PROJECT_DIR` → git root (legacy fallback) → cwd. Note `MEMENTO_PROJECT_HASH` overrides both project and conversation hash, collapsing them — handy for tests, but it masks the distinction.
